...
## Mission
Systematically build, validate, and deploy a portfolio of 5-10 elite, 100% mechanical forex trading strategies through the "Project Playbook Gauntlet" framework. Generate capital via prop firm competitions (zero-risk path) then scale.

## Current Phase
**Optimize & Complete** - Protocol Engine v2.3.4 is operational. Focus: fix remaining issues, complete fundamental protocols, validate with backtests, prep for prop firm.
...
- **Standard Operating Procedure (Save Routine):** 1. Clear all Jupyter cell outputs.
  2. Run `git add .`
...
- P1: Support & Resistance (v3.4) | P2: Psychological Level Liquidity | P3: Candlestick Reversal
...
### Protocol Engine v2.4.0 (OPERATIONAL)
...
**7 Engines:**
1. **COT Processor** — Downloads/caches CFTC data (disagg, tff, legacy), maps CFTC codes, calculates WillCo percentile. Tracks 12 instruments (7 forex, 3 commodities, 2 indices).
2. **FRED Dashboard** — Pulls VIX, HY OAS, US 2Y/10Y, TIPS, Breakeven, USD TWI from FRED API.
3. **Rate Diff Engine** — Calculates 28-pair interest rate differentials (7 majors + 21 crosses) with shift tracking.
4. **Event Calendar** — Forex Factory API integration with country-aware matching, ADP/NFP collision fix, tiered blackout windows.
5. **Session State** — Persists positions, P&L, risk usage, liquidity regime, VIX>25 duration tracking across sessions via JSON.
6. **Conviction Matrix** — Normalizes 6 layers (liquidity, fiscal, rate_diff, risk_sentiment, retail, regime) to SUPPORTS/OPPOSES/NEUTRAL per trade direction using INSTRUMENT_DNA.
7. **Instrument Selector** — Scores all 35 instruments, applies regime permissions, concentration limits, session compatibility. Outputs ranked watchlist.
...
- **Data:** CFTC COT (via CFTC zip files), FRED API, Forex Factory API, central bank rates (manual), retail sentiment (manual from FXSSI/Myfxbook)

### Multi-Chat Architecture (from prior LM Arena sessions)
Three specialized AI chat roles were designed:
1. **Chief of Staff** — System architecture, protocol design, prompt engineering, concept analysis
2. **Jupyter Code Writer** — Protocol Engine development, code review, config verification
3. **Implementation Guide** — Live trading execution, session state, chart scanning, trade management
...
- **Workaround:** Manual GLOBAL_RISK_SENTIMENT override (currently using EXTREME_RISK_OFF)
...
### 9. SEVERE_RISK_OFF Tier (RESOLVED — Implemented in v2.3.4)
- SEVERE_RISK_OFF (VIX 30-40) is now implemented across all 6 code touchpoints: derive_risk_sentiment(), REGIME_PERMISSIONS, _map_regime(), classify_layer(), render_dashboard(), and the 5-tier VIX system
- AUTO thresholds now correct: <16=RISK_ON, 16-20=NEUTRAL, 20-30=MOD, 30-40=SEVERE, >40=EXTREME
- Confirmed by code inspection of Pine Scripts.docx engine copy (April 2026)
- **Status: RESOLVED**

### 10. AUTO Risk Sentiment Thresholds (RESOLVED — Fixed with SEVERE_RISK_OFF)
- derive_risk_sentiment() now uses the correct 5-tier P19-aligned thresholds
- VIX <16=RISK_ON, 16-20=NEUTRAL, 20-30=MOD_RISK_OFF, 30-40=SEVERE_RISK_OFF, >40=EXTREME_RISK_OFF
- **Status: RESOLVED**

### 11. Informed Flow Detection (Moved to P19 Appendix — DRAFT, Unvalidated)
...
### 12. Gauntlet Script Instrument Mode Must Match Pair
...
### 13. Protocol 12 Numbering Conflict (RESOLVED)
- Both Indicator Combination Framework and TTM Squeeze were labeled Protocol 12
- Resolution: P11 = Indicator Combination Framework, P12 = TTM Squeeze
- Current numbering in all docs and TASKS.md reflects this

## Key Files
- `Code for macro analysis.ipynb` — Protocol Engine v2.4.0 (THE core automation)
...
- `DEEP SEARCH PROMPTS OUTPUT.docx` — Weekly macro research output (last: 27-03-2026, Global Liquidity = EXPANSION)
...
- `1 SUPPORT and resistance protocol.docx` — P1 v3.4 (dual-TF regime check, VP primary)
...
- Engine versioning: v2.4.0 (P18 Hybrid Model + VIX duration tracking)
...
## What's Working
- **Protocol Engine v2.4.0** fully operational with 7 engines and HTML dashboard
...
- **Deep Search macro refresh** (last run 27-03-2026, stale)
